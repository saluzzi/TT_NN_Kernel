...
function [y, evalcnt, Samples]=gradient_cross_new_sample(Grids, Bases, soughtfun, tol, lambda, isV0,varargin)
...
while (swp<=nswp)
    ...
    [cry,evalcnt,undersampled,Samples_app] = solve_core(i,SampledCores{i},Bases{i},SampledCores{i+1},lambda,underfactor, X{i},X{i+1},Grids{i},soughtfun,vec,evalcnt,undersampled, [],isV0);
    Samples = [Samples; Samples_app];
    ...
    if (i~=(d-istart+1))
        ...
        [crz,evalcnt,undersampled,Samples_app] = solve_core(i,SampledZZ{i},Bases{i},SampledZZ{i+1},lambda,1, Xz{i},Xz{i+1},Grids{i},soughtfun,vec,evalcnt,undersampled, yz,0);
        Samples = [Samples; Samples_app];
        crz = reshape(crz, rz(i), n(i), rz(i+1), 1);
        err = norm(crz(:))/norm(yz(:));
        if (dir>0)           
            yz = assemble_sampled_tt(SampledCores{i},Bases{i},SampledZY{i+1},yapprox);
            [EnrichCore,evalcnt,undersampled,Samples_app] = solve_core(i,SampledCores{i},Bases{i},SampledZZ{i+1},lambda,1, X{i},Xz{i+1},Grids{i},soughtfun,vec,evalcnt,undersampled, yz,0);
            Samples = [Samples; Samples_app];
            EnrichCore = reshape(EnrichCore, ry(i), n(i), rz(i+1), 1);
        else
            yz = assemble_sampled_tt(SampledZY{i},Bases{i},SampledCores{i+1},yapprox);
            [EnrichCore,evalcnt,undersampled,Samples_app] = solve_core(i,SampledZZ{i},Bases{i},SampledCores{i+1},lambda,1, Xz{i},X{i+1},Grids{i},soughtfun,vec,evalcnt,undersampled, yz,isV0);
            Samples = [Samples; Samples_app];
...
function [cry,evalcnt,undersampled, Samples] = solve_core(i,SampledPrev,Phi,SampledNext,lambda,underfactor, Xprev,Xnext,grid,soughtfun,vec,evalcnt,undersampled,CoreToSubtract,isV0)
...
if isV0
    Samples = [Samples(1:end-1,:); zeros(1,size(Samples,2))];
end
